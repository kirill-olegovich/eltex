...
int main(void) {
	...
	if (bind(socketfd_tcp, (struct sockaddr*)&serveraddr, sizeof(serveraddr)) == -1) {
		perror("bind");
		exit(1);
	}
	
	if (bind(socketfd_udp, (struct sockaddr*)&serveraddr, sizeof(serveraddr)) == -1) {
		perror("bind");
		exit(1);
	}
	...
	while (1) {
        ...
        if (FD_ISSET(socketfd_tcp, &rset)) {
			...
			connectedfd = accept(socketfd_tcp, (struct sockaddr*)&clientaddr, &length);
			...
			char buffer[1024];
			bzero(buffer, 1024);
			recvfrom(socketfd_udp, buffer, 1024, MSG_WAITALL, (struct sockaddr*)&clientaddr, &length);
			printf("From client (%d): %s\n", connectedfd, buffer);
			sendto(socketfd_udp, "OK", 2, MSG_CONFIRM, (struct sockaddr*)&clientaddr, length);
			sleep(2);
