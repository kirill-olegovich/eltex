...
void update_users(struct user user_list[10], char name[20], int qid,
                  int action) {
    if (action == 0) {
        for (int i = 0; i < 10; i++) {
...
int main(int argc, char **argv) {
    ...
    if (pid == 0) {
        while (1) {
            ...
            if (msgrcv(qid, &my_message, sizeof(struct message_text), 0, 0) ==
                -1) {
                perror("msgrcv");
                exit(1);
            }
            ...
            if (strncmp(my_message.message_text.buf, "exit", 4) == 0) {
                update_users(user_list, my_message.message_text.name,
                             my_message.message_text.qid, 1);
            ...
            memcpy(my_message.message_text.user_list, user_list,
                   sizeof(struct user) * 10);
            ...
            for (int i = 0; i < 10; i++) {
                ...
                if (msgsnd(client_qid, &my_message, sizeof(struct message_text),
                           0) == -1) {
                    perror("msgsnd");
                    exit(1);
                }
